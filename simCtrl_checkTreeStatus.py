...
def checkOptions(options):
    ...
    if os.path.exists(os.path.join(options.dir, 'simulationInfo.xml')):
        infoTree = ET.parse(os.path.join(options.dir, 'simulationInfo.xml'))
    ...
    if not os.path.exists(os.path.join(options.dir, 'simulationInfo.xml')):
        sys.stderr.write('%s: Unspecified step size, not found in simulationInfo.xml, using default [].\n' % (sys.argv[0], defaultStepSize))
        options.stepSize = defaultStepSize
    else:
        infoTree = ET.parse(os.path.join(options.dir, 'simulationInfo.xml'))
